...
namespace NamelessMC\DiscordIntegration\Integration;
 ...
 * @license MIT
 */
class DiscordIntegration extends \IntegrationBase implements \HasIntegrationSettings {
    ...
    public function onUnlinkRequest(\User $user) {
        ...
        $roles = array_unique(array_map(static function ($group_id) {
            return DiscordUtils::getDiscordRoleId($group_id);
        }, $user->getAllGroupIds()));

        DiscordUtils::updateDiscordRoles($user, [], $roles);
        ...
        if (\Input::exists()) {
            if (\Token::check()) {
                if (\Input::get('action') === 'integration_settings') {
                    ...
                    \Redirect::to(\URL::build('/panel/core/integrations/', 'integration=' . $this->getName()));
                }
            } else {
